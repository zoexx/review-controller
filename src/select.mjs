...
const TIER_RANK = { 'context-gated': 0, default: 1, 'always-on': 2 };
const SEV_RANK = { blocking: 0, important: 1, nit: 2, suggestion: 3 };

function matchSelector(term, sel) {
  if (sel.startsWith('domain:')) return (term.tags.domains || []).includes(sel.slice(7));
  if (sel.startsWith('layer:')) return term.tags.layers.includes(sel.slice(6));
  return globMatch(term.id, sel);
}
const matchesAny = (term, sels) => (sels || []).some((s) => matchSelector(term, s));
...
export function resolveScope(profile, context) {
  const detected = context.layers;
  if (profile.layers && profile.layers.length) {
    const filtered = detected.filter((l) => profile.layers.includes(l));
    return filtered.length ? filtered : detected;
  }
  return detected;
}

export function selectTerms(dictionary, profile, context) {
